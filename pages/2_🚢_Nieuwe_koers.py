...
small_fam_mapping = {'Ja':1, 'Nee':0}
small_fam_demapping = {1:'Ja', 0:'Nee'}
...
if pagina == 'Data verkenning':
    ...
    train_new['Small_fam'] = train_new['Small_fam'].map(small_fam_demapping)
    ...
    if Variabele == "Type":
        ...
        st.plotly_chart(fig)
    elif Variabele == "Age_categories":
        kleur_dict = {
            'Kind (<16)' :'#FFE599', 
            'Jong volwassen (16<=34)':'orange', 
            'Volwassen (35<=49)':'chocolate', 
            'Middelbare leeftijd (50<=64)':'sienna', 
            'Oudere (>65)':'saddlebrown'}
        Volgorde_dict = {"Age_categories": ["Kind (<16)", "Jong volwassen (16<=34)", 
                                            "Volwassen (35<=49)", "Middelbare leeftijd (50<=64)", 
                                            "Oudere (>65)"]}
        fig = px.histogram(train_new, x=Variabele, title=f"Histogram van {Variabele}",
                        color=train_new['Age_categories'],
                        color_discrete_map=kleur_dict,
                        category_orders=Volgorde_dict)
        st.plotly_chart(fig)
        ...
        '2e klas': '#4682B4',
        '3e klas': '#8B0000'}
        volgorde_dict={'Pclass':['1e klas','2e klas','3e klas']}
        fig = px.histogram(train_new, x=Variabele, title=f"Histogram van {Variabele}",
                        color=train_new['Pclass'],
                        color_discrete_map=kleur_dict,
                        category_orders=volgorde_dict)
        st.plotly_chart(fig)
    else:
    ...
    train_new['Small_fam'] = train_new['Small_fam'].map(small_fam_mapping)
    ...
    def plot_multiple_graphs(data, variables, target='Survived'):
        ...
        fig.update_layout(
            ...
            showlegend=False,
            height=1100,  # Aangepaste hoogte voor de subplots
    ...
    for n in neighbors_range:
        ...
        knn.fit(X_train_knn_scaled, y_train)  # Nu geschaald!
    ...
    best_n = neighbors_range[accuracies_scaled.index(max(accuracies_scaled))]
    st.info(f"Beste aantal buren: {best_n}  \n"
            f"Hoogste nauwkeurigheid: {max(accuracies_scaled)}")
    # Plot de nauwkeurigheid voor verschillende waarden van n_neighbors
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(neighbors_range, accuracies_scaled, marker='o', linestyle='--', color='b')
